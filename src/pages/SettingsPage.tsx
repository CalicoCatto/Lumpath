      ...
      <div className="lg:grid lg:grid-cols-2 lg:gap-8">
        ...
        <div>
          ...
          <section className="mb-6 md:mb-8">
            <label className="text-sm font-medium text-stone-600 mb-2.5 block">模型提供商</label>
                ...
                <button
                  ...
                  className={`rounded-2xl border-2 p-4 text-left transition-all duration-200 ${
                    provider === p.id
                      ...
                      : 'border-stone-200 bg-white hover:border-primary-300 hover:shadow-md'
                  }`}
                >
          ...
          {/* Model Selection */}
          <section className="mb-6 md:mb-8">
            <label className="text-sm font-medium text-stone-600 mb-2.5 block">选择模型</label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
              {models.map(m => (
                <button
                  key={m.id}
                  onClick={() => setModelId(m.id)}
                  className={`w-full rounded-xl border-2 p-3 md:p-3.5 text-left transition-all duration-200 ${
                    modelId === m.id
                      ? 'border-primary-500 bg-primary-50 shadow-sm shadow-primary-500/10'
        ...
        <div>
          {/* API Key */}
          <section className="mb-6 md:mb-8">
            <label className="text-sm font-medium text-stone-600 mb-2.5 block">API 密钥</label>
            <div className="relative">
              <input
                ...
                placeholder="sk-..."
                className="w-full bg-white border border-stone-200 rounded-xl px-4 py-3 pr-10 text-sm outline-none transition-all duration-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-200"
              />
              <button
            ...
            <a
              ...
              className="inline-flex items-center gap-1 text-xs text-primary-600 mt-2 hover:text-primary-700 transition-colors"
            >
          ...
          <section className="mb-6 md:mb-8">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-white border border-stone-200 rounded-xl py-3 text-sm font-medium text-stone-600 disabled:opacity-50 transition-all duration-200 active:bg-stone-50 hover:border-stone-300 hover:shadow-md"
            >
              {testing ? (
                <>
          ...
          <button
            onClick={handleSave}
            disabled={!canSave}
            className="w-full bg-gradient-to-r from-primary-500 to-rose-500 text-white font-semibold rounded-xl py-3.5 text-sm disabled:opacity-50 transition-all duration-300 active:scale-[0.98] hover:shadow-lg hover:shadow-primary-500/20 hover:brightness-105"
          >
            {saved ? '已保存 ✓' : '保存设置'}
          </button>
        </div>
